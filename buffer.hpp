...
/**
 * Temporary function that skips staging buffer and create only on host memory
 * to avoid out of memory on device with VkAllocateMemory (it happened with lucy.obj)
 * The real stuff will be done with vk_mem_alloc, but I want to play with it
 */
template <class T>
void createBufferOnHostMemory(
    Type bufferType,
    VkPhysicalDevice physicalDevice,
    VkDevice logicalDevice,
    VkCommandPool commandPool,
    ...
    auto buffer_usage_bit = (bufferType == Type::Vertex) ? VK_BUFFER_USAGE_VERTEX_BUFFER_BIT : VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
